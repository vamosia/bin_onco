...
while( <IN> ) {
	...
	if( exists $ignore{ $varClass } ) {
	    ...
	    $fix{ no_varClass }++;
	    print NOTFIX "no_varClass | $varClass | $line{ Hugo_Symbol } | $id ($a,$b,$c,$d)\n";

	} elsif ( ! exists $entrez{ $line{ 'Entrez_Gene_Id' } } ) {
	    # this entrez_id, doesn't exits in the CBIO file
	    $fix{ no_entrez }++;
	    print NOTFIX "no_entrez | $varClass | $line{ Hugo_Symbol } | $id ($a,$b,$c,$d)\n";
