...
my $dbh = DBI->connect("dbi:Pg:dbname=$options{ -db };host=localhost;port=5432",'postgres','ionadmin',{AutoCommit=>0,RaiseError=>1,PrintError=>0});
...
foreach( sort { $a <=> $b } keys %{ $dbpriority } ) {
    ...
    # Importing seed DB
    if( $options{ -seed } ) {
	next unless( exists $seed_table{ $table } );
	$file =  "data_${table}.psv";
    }
...
sub process_file {
    ...
    my $stmt = qq( INSERT INTO gene_alias( entrez_gene_id, gene_alias ) VALUES (?,?) );
	...
	my $rv = $sth->execute( $l[0], $l[1] ) or die $DBI::errstr;
